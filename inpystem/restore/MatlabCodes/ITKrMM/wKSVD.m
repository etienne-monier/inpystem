function [Dictionary,output] = wKSVD(...
    ...
    for iterNum = 1:param.numIteration
        ...
        if param.save_it
            [status, msg] = mkdir('save_it');
            save(['save_it/it_' num2str(iterNum) '.mat'], 'tmp_out');
        end

        if (iterNum>1 & param.displayProgress)

            output.totalerr(iterNum-1) = sqrt(sum(sum((Mask.*(Data-tmp_out)).^2))/prod(size(Data)));
...
function res = sec2str(t)
    %   Writes the time t from float in seconds to human readable time.
